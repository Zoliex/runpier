...
var runNetwork = function anonymous(input) {
  var net = {
    layers: [
      ...
      {
        0: {
          bias: 0.30606978889779535,
          weights: {
            r: 0.8381133171486668,
            g: -0.8602486258125565,
            b: -1.5270511521731682,
          },
        },
        1: {
          bias: 3.1329999860658906,
          weights: {
            r: 2.2463466077278498,
            g: -1.1545550503755377,
            b: -4.0279511285495255,
          },
        },
        2: {
          bias: 1.122002476278899,
          weights: {
            r: 1.2189688760097008,
            g: -0.7477856700925529,
            b: -2.253326449942745,
          },
        },
      },
      {
        black: {
          bias: 3.324204314263548,
          weights: {
            0: -1.7719498459629373,
            1: -5.890287515022658,
            2: -2.878376676040711,
          },
        },
...
socket.on("res_usb", async function (data) {
  ...
  for (const device of data) {
    ...
    var type = types.filter((item) => item.type === device.type);
    if (type[0].icon == undefined) {
      type[0].icon = "";
    }
    html =
      html +
      `<tr><td class="icon-name"><div style="background: #ff6000;" class="icon"><i style="${textColor(
        "#ff6000"
      )};" class="${type[0].icon}"></i></div><span>${
        device.name
      }</span></td><td>${device.id}</td><td>${device.vendor}</td><td>${
        device.type
      }</td><td>${device.maxPower}</td></tr>`;
    if (device.maxPower != null) {
      power += parseInt(device.maxPower.replace("mA", "") || "0");
    }
  }
